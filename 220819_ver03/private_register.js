        ...
        if (user) {
            ...
            docRef.get().then((doc) => {
                ...
                // console.log("Document data:", doc.data());
                    ...
                    if(doc.data().level > 1){
                        const res_btn = document.querySelector("#header_res_btn");
                        const res_img = document.querySelector("#input_timetable_img");
                        const del_data = document.querySelector("#del_data");
                        del_data.style.cssText ="";
                        res_img.style.cssText ="";
                        res_btn.style.cssText ="";
    ...
    snapshot.forEach((item)=> {
        ...
        const template = `
                <tr>
                    ...
                    <td>${class_time}</td>
                    <td>${class_name}</td>
                    <td><button type="button" class="btn btn-dark" style="font-size: smaller; font-weight: bold;"
                    id="${item.id}"
    ...
    document.getElementById('save_reservation').addEventListener('click',(e) => {
        ...
        console.log(res_time);
            ...
            .then((docRef) => {
                console.log("Document written with ID: ", docRef.id);
                alert('예약되었습니다!');
                window.location.reload(); // 예약 후 새로고침
            })
            .catch((error) => {
                console.error("Error adding document: ", error);
            });
