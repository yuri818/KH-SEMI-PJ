...
//////////////////////////***** 로그인한 아이디 레벨 확인, 권한 부여 *****//////////////////////////
          ...
          docRef
            ...
            .then((doc) => {
              if (doc.exists) {
                // 회원은 가입 시 자동으로 레벨 1, 관리자는 레벨 5를 부여하여 회원과 구분함
                if (doc.data().level > 1) { 
      ...
      for (
        ...
        const template = `
          <tr>
              ...
              <td style="width: 5rem"><a class="cs_title" href="./cs_edit_personal.html?id=${
                snapshot.docs[i].id
              }">
                ${snapshot.docs[i].data().subject}
              </a></td>
              <td style="width: 10%">${snapshot.docs[i].data().writer}</td>
              <td class="write_d" style="width: 20%">${
                snapshot.docs[i].data().write_date
              }</td>
            </tr>
          `;
